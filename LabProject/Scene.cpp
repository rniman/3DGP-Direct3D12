...
//	d3dRootSignatureDesc.NumStaticSamplers = 0; d3dRootSignatureDesc.pStaticSamplers = nullptr;
...
ID3D12RootSignature* CScene::GetGraphicsRootSignature() const
{
...
void CScene::CreateGraphicsPipelineState(ID3D12Device* pd3dDevice)
{
	...
	D3DCompileFromFile(L"Shaders.hlsl", nullptr, nullptr, "VSMain", "vs_5_1", nCompileFlags, 0, &pd3dVertexShaderBlob, nullptr);
	D3DCompileFromFile(L"Shaders.hlsl", nullptr, nullptr, "PSMain", "ps_5_1", nCompileFlags, 0, &pd3dPixelShaderBlob, nullptr);
...
void CScene::ReleaseObjects()
{
	...
	if (m_ppShaders)
	{
		for (int i = 0; i < m_nShaders; i++)
		{
			...
			m_ppShaders[i]->ReleaseObjects(); m_ppShaders[i]->Release();
		}
		delete[] m_ppShaders;
	}
}
...
void CScene::UpdateObjects(const float deltaTime)
{
	for (int i = 0; i < m_nShaders; i++)
	{
		m_ppShaders[i]->UpdateObjects(deltaTime);
	}
}
