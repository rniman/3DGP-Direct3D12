...
CHeightMapImage::CHeightMapImage(LPCTSTR pFileName, int nWidth, int nLength, XMFLOAT3 xmf3Scale)
{
	...
	HANDLE hFile = CreateFile(pFileName, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_ATTRIBUTE_READONLY, nullptr);
	DWORD dwBytesRead;
...
XMFLOAT3 CHeightMapImage::GetHeightMapNormal(int x, int z)
{
	...
	//높이 맵에서 (x, z) 좌표의 픽셀 값과 인접한 두 개의 점 (x+1, z), (x, z+1)에 대한 픽셀 값을 사용하여 법선 벡터를 계산한다.
	int nHeightMapIndex = x + (z * m_nWidth);
	int xHeightMapAdd = (x < (m_nWidth - 1)) ? 1 : -1;	//x가 전체 가로 크기보다 작으면 1, 아니라면 -1 -> 근데 위에서 이건 걸러지지않나?
	int zHeightMapAdd = (z < (m_nLength - 1)) ? m_nWidth : -m_nWidth; // z가 전체 세로보다 작으면 m_nWidth
	...
	: CMesh(pd3dDevice, pd3dCommandList)
{
	...
	사각	형 줄의 개수는 (nLength-1)이고 한 줄에서 (nWidth*2)개의 인덱스를 갖는다. 그리고 줄이 바뀔 때마다 인덱스를
	하나 추가하므로 (nLength-1)-1개의 인덱스가 추가로 필요하다.*/
